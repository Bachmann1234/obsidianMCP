...
class TestObsidianMCPServer:
    ...
    async def test_initialize_index_full_rebuild(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with (
                patch.object(server.search_index, "needs_update", return_value=True),
                patch.object(
                    server.search_index,
                    "incremental_update",
                    return_value={"updated": 0, "added": 0},
                ),
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with (
                ...
                patch.object(
                    server.search_index,
                    "incremental_update",
                    return_value={"updated": 2, "added": 1},
                ),
            ):
                await server._initialize_index()
    ...
    async def test_search_notes_with_tags(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with patch.object(
                server.search_index, "search", return_value=mock_results
            ) as mock_search:
                await server._search_notes(
                    {"query": "test", "tags": ["python"], "limit": 5}
                )

                mock_search.assert_called_once_with(
                    query="test", limit=5, tags={"python"}
                )
    ...
    async def test_get_note_by_path(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with (
                patch.object(
                    server.search_index, "get_note_by_path", return_value=mock_result
                ),
    ...
    async def test_get_note_not_found(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with (
                patch.object(
                    server.search_index, "get_note_by_path", return_value=None
                ),
    ...
    async def test_get_note_by_title_search(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with (
                ...
                patch.object(
                    server.search_index, "search", return_value=[mock_search_result]
                ),
            ):
    ...
    async def test_list_recent_notes(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with patch.object(
                server.search_index, "get_recent_notes", return_value=mock_recent
            ):
    ...
    async def test_get_all_tags(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with patch.object(
                server.search_index, "list_all_tags", return_value=mock_tags
            ):
    ...
    async def test_search_by_tag_multiple(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with patch.object(
                server.search_index, "search", return_value=[]
            ) as mock_search:
                ...
                mock_search.assert_called_once_with(
                    query="tags:python OR tags:testing", limit=10
                )
    ...
    async def test_get_vault_stats(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with (
                patch.object(
                    server.search_index, "get_stats", return_value=mock_index_stats
                ),
                patch.object(
                    server.watcher, "get_stats", return_value=mock_watcher_stats
                ),
                patch.object(
                    server.parser,
                    "discover_notes",
                    return_value=["note1.md", "note2.md"],
                ),
            ):
    ...
    async def test_get_vault_stats_error(self, server_config):
        ...
        with patch("obsidian_mcp.server.Server"):
            ...
            with patch.object(
                server.search_index, "get_stats", side_effect=Exception("Index error")
            ):
                result = await server._get_vault_stats({})
    ...
    async def test_run_with_watcher(self, server_config):
        ...
        with (
            patch("obsidian_mcp.server.Server") as mock_server_class,
            ...
            with (
                patch.object(server.watcher, "start") as mock_start,
...
class TestServerMain:
    ...
    def test_main_with_vault_path(self, temp_vault):
        ...
        with (
            patch("obsidian_mcp.server.ObsidianMCPServer") as mock_server_class,
