...
def test_vault_watcher_update_note_parse_failure(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_update_note_exception(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_on_moved(temp_vault, mock_parser, mock_search_index):
    ...
    with patch.object(watcher, '_update_note') as mock_update:
        ...
        callback.assert_called_once_with('moved', Path(mock_event.src_path), Path(mock_event.dest_path))


def test_vault_watcher_on_moved_to_non_markdown(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_manager_initialization(temp_vault, mock_parser, mock_search_index):
    """Test VaultWatcherManager initialization."""
    manager = VaultWatcherManager(temp_vault, mock_parser, mock_search_index, enabled=True)
...
def test_vault_watcher_manager_disabled(temp_vault, mock_parser, mock_search_index):
    ...
    manager = VaultWatcherManager(temp_vault, mock_parser, mock_search_index, enabled=False)
    
    manager.start()
    assert manager.watcher is None


def test_vault_watcher_manager_start_success(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_manager_stop_when_not_started(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_manager_on_change_stats(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_manager_context_manager(temp_vault, mock_parser, mock_search_index):
...
def test_vault_watcher_start_stop_watching(temp_vault, mock_parser, mock_search_index):
    ...
    with patch('obsidian_mcp.watcher.Observer') as mock_observer_class:
        ...
        mock_observer.schedule.assert_called_once_with(watcher, str(temp_vault), recursive=True)
...
def test_vault_watcher_stop_watching_when_not_started(temp_vault, mock_parser, mock_search_index):
